...
def checkKey(url):
	...
	request=urllib2.Request(url)
        request.add_header("User-agent","Mozilla/5.0 (Macintosh; Intel Mac OS X 10_6_8) AppleWebKit/534.30 (KHTML, like Gecko) Chrome/12.0.742.112 Safari/534.30" )
			...
			try:
				...
				#page = commands.getoutput('curl --silent -L -A "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_6_8) AppleWebKit/534.30 (KHTML, like Gecko) Chrome/12.0.742.112 Safari/534.30" "'+url+'"')
                		request=urllib2.Request(url)
                                request.add_header("User-agent","Mozilla/5.0 (Macintosh; Intel Mac OS X 10_6_8) AppleWebKit/534.30 (KHTML, like Gecko) Chrome/12.0.742.112 Safari/534.30" )
                                response=urllib2.urlopen(request)
                                #Corren: removed call to simplejson
				#json = simplejson.loads(page)
                                #Corren: Convert response to dict
